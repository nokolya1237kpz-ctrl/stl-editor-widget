...
/** 2D PNG-превью отключено: основной контроль результата теперь через 3D-стол и скриншот. */
...
/** Полностью очищает 3D-стол и текущий файл, чтобы начать новую модель без перезагрузки страницы. */
function clearTable() {
  if (currentMesh) {
    scene.remove(currentMesh);
    if (currentMesh.geometry) currentMesh.geometry.dispose();
  ...
  if (analyzeOut) analyzeOut.classList.add('hidden');
  const advisorOut = document.getElementById('advisorOut');
  if (advisorOut) advisorOut.classList.add('hidden');
  const orientCtrl = document.getElementById('orientControls');
  if (orientCtrl) orientCtrl.classList.add('hidden');
  const editorCtrl = document.getElementById('editorControls');
  if (editorCtrl) editorCtrl.classList.add('hidden');
  const svgInfo = document.getElementById('svgFileInfo');
  if (svgInfo) svgInfo.classList.add('hidden');
  const stlInput = document.getElementById('stlFile');
  if (stlInput) stlInput.value = '';
  const printInput = document.getElementById('printStlFile');
  if (printInput) printInput.value = '';
  const svgInput = document.getElementById('svgFile');
  if (svgInput) svgInput.value = '';
  showStatus('✅ Стол очищен');
}

/** Сохраняет PNG-скриншот текущего 3D-стола. */
function takeTableScreenshot() {
  if (!renderer || !scene || !camera) {
    showStatus('❌ 3D-просмотр ещё не готов', true);
    return;
  }
  try {
    if (controls) controls.update();
    ...
    a.download = `stl-master-table-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.png`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    showStatus('📸 Скриншот стола сохранён');
  } catch (e) {
    console.error('Ошибка скриншота:', e);
    showStatus('❌ Не удалось сделать скриншот', true);
  }
}
